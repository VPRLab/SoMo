...
from solmoctor.core.cfg import EdgeFlag, ICFG_SSA, ICFGEdge, ICFGNode, FunctionSinkNode, ModifierSinkNode, SlitherLValue
from solmoctor.core.cfg.icfg_types import CallReturnEdge, FunctionCallEdge, GeneralEdge
from solmoctor.core.mdg.utils import NodeEdgeProcessor, PropagateDirection, ReadWriteMapper, TEdgeMap, FunctionSinkToConstructorTracer, ConditionOperationChecker
from solmoctor.core.cfg.defined_node import EntryPoint, ExitPoint, StateVariableWrapper
from solmoctor.core.mdg.analyze_result import ModifierSinkAnalyzingResult

from slither.core.variables.state_variable import StateVariable
from slither.core.variables.local_variable import LocalVariable
from slither.core.declarations import Contract, FunctionContract, SolidityVariableComposed

from slither.slithir.operations import Operation as SlitherIR
from slither.slithir.operations import Return, Assignment, Binary, SolidityCall, EventCall, Index
...
TUsedVariable: T.TypeAlias = T.Union[StateIRVariable, LocalVariable, TemporaryVariableSSA, ReferenceVariableSSA, TupleVariableSSA, TupleVariable]
# The working tuple that are pushed and popped from the running stack.
# Using a stack to record the return sites
TWorkingTuple: T.TypeAlias = T.Tuple[ICFGNode, T.List[TUsedVariable], PropagateDirection, T.List[ICFGNode]]
# Using for later checking. 
TFunctionTaintSequence: T.TypeAlias = T.List[ICFGNode]
...
class FunctionTaintAnalyzer:
    def __init__(self) -> None:
        ...
        self._ir_non_ssa_mapper: T.Callable[[T.Union[SlitherIR, LocalIRVariable]], StateVariable] = lambda ir: ir.non_ssa_version
    ...
    def _process_working_node(
        ...
        if isinstance(working_node, T.Union[ExitPoint, EntryPoint]) or str(working_node) == "END_IF" or str(working_node) == "_":
            graph.nodes[working_node]['is_function_taint'] = True
            self._append_function_sink_source(working_node, function_sink, graph)
            return True
        ...
        if isinstance(working_node, SlitherNode):
            return False
    ...
    def _search_function_sinks(
        ...
        while running_stack:
            ...
            if isinstance(working_node, FunctionSinkNode):
