...
class ConditionalBlockLocator:
    ...
    def _extract_if(self, node: SlitherNode) -> T.Optional[Expression]:
        if node.type == SlitherNodeType.IF:
            return node.expression
        else:
            return None

    def _extract_require(self, node: SlitherNode) -> T.Optional[CallExpression]:
    ...
    def is_conditional_node(self, node: SlitherNode) -> T.Union[bool, T.Tuple[Expression, CallExpression]]:
        """
        Check whether a given SlitherNode contains conditional statement or not.
    ...
    def extract_one_node(self, node: SlitherNode) -> T.Union[CallExpression, Expression, None]:
    ...
    def extract(self, inputs: T.Union[Modifier, FunctionContract]) -> T.List[T.Union[Expression, CallExpression]]:
        nodes: T.List[SlitherNode] = inputs.nodes_ordered_dominators
