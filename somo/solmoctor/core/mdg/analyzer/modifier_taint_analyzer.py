...
from slither.slithir.variables import StateIRVariable, LocalIRVariable, TemporaryVariableSSA, Constant
...
from solmoctor.core.mdg.utils import TEdgeMap, PropagateDirection, NodeEdgeProcessor, ConditionOperationChecker, ParameterChecker
from solmoctor.core.cfg import ICFG_SSA, EntryPoint, ExitPoint, EdgeFlag, ICFGNode, ICFGEdge, ModifierSinkNode,\
...
class ModifierTaintAnalyzer:
    ...
    def _propagate_taint(self, modifier_sink: ModifierSinkNode, modifier_entry: EntryPoint, graph: nx.MultiDiGraph, modifier: Modifier) -> None:
        ...
        while running_stack:
            ...
            # Generally the `SlitherNode` could only be `_` or `END_IF`
            # Process the working node
            # Only process the node that writes to some variable.
            if not isinstance(working_node, SlitherNode) and hasattr(working_node, "lvalue"):
