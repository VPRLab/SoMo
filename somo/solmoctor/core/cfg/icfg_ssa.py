...
from solmoctor.utils import ExternalCallIdentifier
...
from slither.slithir.operations import (LibraryCall, InternalCall, HighLevelCall)


class ICFG_SSA:
    def __init__(self, origin: Contract, slither_obj: Slither = None, build_special_constructor: bool = True) -> None:
        ...
        self._has_ssa: T.Callable[[SlitherNode], bool] = lambda node: True if node.irs_ssa else False
    ...
    def _create_entry_exit_point(self):
        ...
        for function in regular_functions:
            ...
            origin_entry: SlitherNode = function.entry_point
            node_after_entry: SlitherNode = origin_entry.sons[0]
    ...
    def _add_one_node_to_graph(self, node: SlitherNode, scope: str, graph: nx.MultiDiGraph):
    ...
    def _add_general_edge(self, node: SlitherNode, graph: nx.MultiDiGraph):
        ...
        if self._has_ssa(node):
            ...
            src_node: SlitherIR = node.irs_ssa[-1]
        else:
    ...
    def _add_function_call_edges(self, node: SlitherNode, graph: nx.MultiDiGraph):
        if not self._has_ssa(node):
            ...
            for index in range(len(node.irs_ssa)):
                ...
                current_ir_node: SlitherIR = node.irs_ssa[index]

                # if isinstance(current_ir_node, T.Union[HighLevelCall, LibraryCall, InternalCall]):
                if isinstance(current_ir_node, T.Union[LibraryCall, InternalCall]):
                    ...
                    return_dst_node: T.Union[SlitherNode, SlitherIR] = self._get_node_general_son(current_ir_node, graph)
    ...
    def _get_node_general_son(self, node: SlitherIR, graph: nx.MultiDiGraph) -> T.Union[SlitherIR, SlitherNode]:
    ...
    def _add_edge(self, node: SlitherNode, graph: nx.MultiDiGraph):
